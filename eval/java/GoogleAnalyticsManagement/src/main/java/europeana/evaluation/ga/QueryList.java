...
public class QueryList<T extends BasicQuery> {
	...
	public void Filter_SimpleQueries(){
		this.setQueries(queries.stream().filter(p -> (p.getQuery().matches("[\"\'\\“A-Za-z\\s]+") && !p.getQuery().contains("AND") && !p.query.contains("OR") && p.getQuery().length() > 2)).collect(Collectors.toList()));
	}
