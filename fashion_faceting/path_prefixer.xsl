...
<xsl:stylesheet
    ...
    <xsl:variable name="colors" select="distinct-values(doc('fash_thesaurus.rdf.xml')/rdf:RDF/rdf:Description/skos:inScheme[@rdf:resource eq 'http://thesaurus.europeanafashion.eu/thesaurus/Colours']/parent::rdf:Description/@rdf:about)" as="xs:string*"/>
    <xsl:variable name="materials" select="distinct-values(doc('fash_thesaurus.rdf.xml')/rdf:RDF/rdf:Description/skos:inScheme[@rdf:resource eq 'http://thesaurus.europeanafashion.eu/thesaurus/Materials']/parent::rdf:Description/@rdf:about)" as="xs:string*"/>
    <xsl:variable name="techniques" select="distinct-values(doc('fash_thesaurus.rdf.xml')/rdf:RDF/rdf:Description/skos:inScheme[@rdf:resource eq 'http://thesaurus.europeanafashion.eu/thesaurus/Techniques']/parent::rdf:Description/@rdf:about)" as="xs:string*"/>
    <xsl:template match="/">
        <add>
            <xsl:for-each select="//doc">
                <doc>
                    <xsl:variable name="is-fash-doc"
                        select="exists(field[@name eq 'provider_aggregation_edm_provider'][text() eq 'Europeana Fashion'])"
                        as="xs:boolean"/>
                    <xsl:for-each select="element()">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="$name eq 'skos_concept'">
                                <xsl:for-each select="str">
                                        <xsl:if test="matches(text(), concat($cv-ns, '/[\\d]+'))">
                                            <xsl:variable name="prefixed-concept">
                                                <xsl:value-of select="concat('¦fashion¦object_type¦', text())"></xsl:value-of>
                                            </xsl:variable>
                                            <field name="SUBJECT">
                            ...
                            <!-- Various Fashion profile fields are mapped to format. Note that they 
                                 ...
                                 the original fields -->
                            <xsl:when test="$name eq 'proxy_dc_format.def'">
                                ...
                                <xsl:for-each select="str">
                                    ...
                                    <field name="proxy_dc_format.def"><xsl:value-of select="text()"/></field>
                                <!-- now let's correct the original fields -->
                                    ...
                                    <xsl:variable name="en-translation">
                                        <xsl:value-of select="doc('fash_thesaurus.rdf.xml')/rdf:RDF/rdf:Description[@rdf:about eq $url]/skos:prefLabel[@xml:lang eq 'en']/text()"/>  
                                    </xsl:variable> 
                                    <field name="proxy_dc_format.en"><xsl:value-of select="concat($label-prefix, $en-translation)"/></field>
                                </xsl:for-each>
                            </xsl:when>
                            <xsl:when test="$name eq 'proxy_dc_format.def' or $name eq 'proxy_dc_format.en'">
