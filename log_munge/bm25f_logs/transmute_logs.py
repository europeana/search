...
rawsearch = Search(using=client, index="logstash-*")
basesearch = rawsearch.query(Q("match", message='"Result rank"')).query('bool', filter=Q("exists", field="session_id"))
start = "2017-03-01T00:00:00.005Z"
current_date = datetime.datetime.strptime(start, "%Y-%m-%dT%H:%M:%S.%fZ")
while(current_date < datetime.datetime.now()):
    ...
    next_date = current_date + datetime.timedelta(minutes=360)
    ...
    for hit in initsearch.scan():
        ...
        augpath = "*" + record
        memory = "100"
        recordsearch = rawsearch.query(Q("match", session_id=session_id)).query(~Q("match", message="Search interaction")).query("match", path=augpath)[0]
